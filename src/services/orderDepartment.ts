    ...
    try {
        return await getManager().transaction(async (manager) => {
            ...
            for await (const item of items){
                ...
                const bcTransaction = await createTransaction(senderId.toString(),'',transaction.inventory1.id.toString(),transaction.inventory2.id.toString(),transaction.item.id.toString(),item.amount,transaction.date.toISOString());
