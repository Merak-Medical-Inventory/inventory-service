...
export const findPresentation = async (id: number) => {
    try {
        const presentation = await getConnection()
            .createQueryBuilder()
            .select('Presentation')
...
export const updatePresentation = async (id: any, dataToUpdate: any) => {
    try {
        ...
        if(update.affected === 0) throw new ErrorHandler(404, 'Presentation not found');
